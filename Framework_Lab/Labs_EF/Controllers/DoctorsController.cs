...
namespace Labs_EF.Controllers
{
    ...
    public class DoctorsController : ControllerBase
    {
        ...
        [HttpGet("Get_Doctors_Speciality")]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public ActionResult<IEnumerable<Get_Doctors_Result_DTO>> Get_Doctors_Speciality(string speciality)
        {
            try
            {
                var result = _Doctors_Services.Find_Doctors_Speciality(speciality);
                return Ok(result);
            }
            catch (Exception exception)
            {
        ...
        [HttpGet("Get_all_Specialities")]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<string>>> Get_Doctors_Speciality_Information()
        {
            try
            {
                var result = await _Doctors_Services.Get_all_Specialities();
                return Ok(result);
            }
            catch (Exception exception)
            {
